		...
		>
			...
			>
				<div className="relative aspect-video overflow-hidden bg-background">
					...
					{faviconUrl && (
						<Container
							roundedSize="xl"
							transparency="lg"
							className="absolute bottom-3 left-3 size-8 overflow-hidden shadow-md shrink-0"
						>
							<img
				...
				<div className="flex-1 p-4">
					<h3 className="text-lg font-bold text-text transition-colors group-hover:text-text/70">
						{project.title}
					</h3>
				...
				<div className="flex items-end justify-between gap-2">
					<div className="flex min-w-0 flex-wrap gap-1.5">
						{(project.tags ?? []).map((tag: string) => (
							<Badge
								key={tag}
								className="bg-neutral/10 border-none text-neutral/80 hover:bg-neutral/10"
							>
								{tag}
							</Badge>
						))}
